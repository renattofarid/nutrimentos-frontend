...
export default function PaymentConceptActions({ hasSelection = false, onNew, onCreatePaymentConcept, onEdit, onDelete }: Props) {
  const { activeTabId, closeTab } = useWindowManager();
  const handleCerrar = () => { if (activeTabId) closeTab(activeTabId); };
  const handleNew = onNew ?? onCreatePaymentConcept ?? (() => {});
