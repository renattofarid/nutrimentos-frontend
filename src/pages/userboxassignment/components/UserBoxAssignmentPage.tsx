...
export default function UserBoxAssignmentPage() {
  ...
  return (
    <PageWrapper>
      ...
      {openCreate && (
        <UserBoxAssignmentModal title={`Crear ${MODEL.name}`} mode="create" open={true} onClose={() => setOpenCreate(false)} />
      )}
      {editId !== null && (
        <UserBoxAssignmentModal id={editId} open={true} onClose={() => setEditId(null)} title={`Editar ${MODEL.name}`} mode="update" />
      )}
      {deleteId !== null && (
